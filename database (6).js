...
db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS add_product_sessions (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id TEXT NOT NULL,
        guild_id TEXT NOT NULL,
        channel_id TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS edit_product_sessions (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id TEXT NOT NULL,
        guild_id TEXT NOT NULL,
        channel_id TEXT NOT NULL,
        step TEXT DEFAULT 'select',
        product_id TEXT,
        created_at INTEGER DEFAULT (strftime('%s', 'now')),
        UNIQUE(user_id, guild_id)
    );

    CREATE TABLE IF NOT EXISTS delete_product_sessions (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id TEXT NOT NULL,
        guild_id TEXT NOT NULL,
        channel_id TEXT NOT NULL,
...
db.getProductKeys = (productId) => {
    return db.prepare('SELECT * FROM product_keys WHERE product_id = ? AND is_used = 0').all(productId);
};
...
db.useProductKey = (productId, userId) => {
    const key = db.prepare('SELECT * FROM product_keys WHERE product_id = ? AND is_used = 0 LIMIT 1').get(productId);
    if (key) {
...
db.createOrder = (userId, productId, ltcAddress, amount, usdAmount = null, guildId = null, sellerId = null) => {
    ...
    db.prepare(`
        INSERT INTO orders (order_id, guild_id, user_id, seller_id, product_id, ltc_address, amount, usd_amount)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(orderId, guildId, userId, sellerId, productId, ltcAddress, amount, usdAmount);
...
db.deliverOrder = (orderId, key) => {
    db.prepare('UPDATE orders SET status = \'delivered\', delivered_key = ?, delivered_at = strftime(\'%s\', \'now\'), updated_at = strftime(\'%s\', \'now\') WHERE order_id = ?').run(key, orderId);
    return db.getOrderById(orderId);
};
    ...
    while (attempts < 100) {
        try {
            db.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `).run(ticketId, guildId, channelId, userId, type, sellerId, productName, paymentMethod);
